...
def install_pip():
    print('Installing pip via easy_install3 command...')
    os.system('mkdir -p  /usr/local/lib/python3.6/site-packages/')
    os.system('easy_install-3 pip >/dev/null')
...
def configurate():
    ...
    for i in range(3):
        os.chmod('/' + file_list[i], 0o700)
    for i in range(3, 6):
        os.chmod('/' + file_list[i], 0o755)
        # os.chmod('/' + file, 0o700)
